...
// 스모크는 그 파일을 truncate 해서 구간을 나눈다.
//
// 서비스워커를 등록하고 워커가 **스스로** /from-sw 를 부른다 — page 타깃의 Fetch 인터셉트가
// 못 보던 자리다(#122). 확장 규칙이 그 요청도 막는지가 이 픽스처의 요점이다(#178).
//
...
<p id=sw>pending</p>
<script>
fetch('/api/user').then(r => r.text()).then(t => { document.getElementById('api').textContent = t; })
  .catch(() => { document.getElementById('api').textContent = 'ERR'; });
navigator.serviceWorker.register('/sw.js').then(() => navigator.serviceWorker.ready)
  .then(() => { document.getElementById('sw').textContent = 'ready'; })
  .catch(e => { document.getElementById('sw').textContent = 'sw-error ' + e.message; });
</script>`;

// install 때 워커가 직접 두 요청을 보낸다. 한쪽(/from-sw)은 규칙에 걸리고 다른쪽(/sw-ok)은
// 통과해야 한다 — 워커가 아예 못 뜬 경우와 규칙이 막은 경우를 가르기 위해서다.
const SW = `self.addEventListener('install', e => {
  e.waitUntil(Promise.allSettled([fetch('/from-sw'), fetch('/sw-ok')]).then(() => self.skipWaiting()));
});`;
...
const server = http.createServer((req, res) => {
  ...
  if (p === '/from-sw' || p === '/sw-ok') { res.end('ok'); return; }
