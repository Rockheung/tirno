...
//
// #183 이후 이 경로는 `--synthetic` 이다. 기본 경로는 실제 마우스라 포커스가 저절로
// 옮겨가고, 대신 가림을 판정한다 — 그쪽은 click-hit-test.test.ts.
...
test('ref 클릭은 클릭보다 먼저 포커스를 옮긴다', async () => {
  ...
  await clickByRef(page, 42, { label: '@42', synthetic: true });
  ...
  assert.match(sent[0], /this\.focus\(\)/, '첫 호출에서 누른 요소에 포커스를 준다 (el 에 call 로 묶인다)');
  assert.match(sent[1], /el\.click\(\)/, '두 번째가 클릭이다');
...
test('dbl 도 같은 순서다', async () => {
  const { page, sent } = fakePage();
  await clickByRef(page, 42, { label: '@42', synthetic: true, dbl: true });
  assert.match(sent[0], /\.blur\(\)/);
  assert.match(sent[0], /this\.focus\(\)/);
  assert.match(sent[1], /el\.click\(\); el\.click\(\);/);
});
