...
export async function clickElement(page: Page, el: ElementHandle<Element>, opts: ClickOptions): Promise<void> {
  if (opts.synthetic) {
...
export async function fillElement(
  page: Page, el: ElementHandle<Element>, value: string, opts: FillOptions,
): Promise<void> {
  const verify = opts.verify !== false;
  // puppeteer 는 요소를 첫 인자로 넘긴다 — `this` 가 아니다.
