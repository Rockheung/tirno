...
**여러 Chrome 을 세션으로 굴리는 CLI.** puppeteer-core 로 띄우고 붙되 필요한 자리는 CDP 를
직접 부르고, 안에 LLM 이 없다 —
...
| **raw CDP** 위에 서기 | 기동·연결·입력·스크린샷·에뮬레이션은 puppeteer-core 의 Page/Browser API 다(호출 ~130곳). CDP 를 직접 부르는 자리는 25개 메서드·37줄로 보조다. puppeteer 가 넣는 기본 플래그와 싸운 흔적(#33·#113, 번역 UI)이 그 증거다. 단계별 탈피는 #182 | `grep` 실측 2026-09-16 |
